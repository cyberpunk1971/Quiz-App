...
function checkAnswerEvent() {
    $('.js-check-answer').on('click', function(event) {
        ...
        if (answerCorrect === true) {
            ...
            $('#display-right-or-wrong').html('Wrong! The correct answer is ' + obj.answer[obj.correctAnswer - 1].value);
        }
    });
}
...
function generateAnswersHTML(answers) {
    ...
    for (i = 0; i < answers.length; i++) {
        let item = answers[i];
        answerList += '<input type="radio" name="q-one" value="' + item.number + '"><label for="q-one">' + item.value + '</label>';
    }
